...
const TokoListPerMemberContent = (props: {
  ownerId?: string;
  ownerNama?: string;
}) => {
  ...
  const data = ownerId ? allData.filter((t) => t.ownerId === ownerId) : allData;
  ...
  const openAdd = () => {
    setAddForm(
      ownerId && ownerNama
        ? { ...emptyForm, ownerId, nama: ownerNama }
        : emptyForm,
    );
    setAddOpen(true);
  };
  ...
  const handleDelete = async (e: React.MouseEvent, id: number) => {
    ...
    if (!ok) return;
    setData(allData.filter((row) => row.id !== id));
    setSelected(null);
...
export default TokoListPerMemberContent;
